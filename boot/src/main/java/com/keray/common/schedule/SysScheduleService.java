package com.keray.common.schedule;
 ...
 */
@Aspect
@Configuration(value = "sysScheduleService")
@Slf4j
@ConditionalOnProperty(value = "keray.schedule.open", havingValue = "true")
@ConfigurationProperties(prefix = "keray.schedule")
public class SysScheduleService implements BSService<SysScheduleModel, Long> {
    @Getter
    @Setter
    ...
    @Resource
    private RedissonLock distributedLock;
    ...
    public void scheduleInit(ApplicationStartedEvent startedEvent) {
        SysThreadPool.execute(() -> {
            List<SysScheduleModel> sysScheduleModels = selectList(
                    Wrappers.lambdaQuery(new SysScheduleModel())
                            .eq(SysScheduleModel::getNamespace, namespace)
         ...
         * 凌晨扫描的任务只获取等待提交的任务
         */
        String cron = String.format("0 0 %d * * ? ", namespace.hashCode() & 7);
        SysThreadPool.taskScheduler.schedule(this::dayScanSchedule, new CronTrigger(cron));
        log.info("提交任务cron检测任务 cron={}", cron);
    }
    ...
    public void deleteFinishData() {
        delete(Wrappers.lambdaUpdate(new SysScheduleModel())
                .and(w -> w.eq(SysScheduleModel::getStatus, ScheduleStatus.success)
                        .or()
                        .eq(SysScheduleModel::getStatus, ScheduleStatus.cancel)
                )
    ...
    public boolean dataScheduleSubmit(SysScheduleModel model, boolean nowExec) {
        ...
        if (sysScheduleMapper.update(newModel,
                Wrappers.lambdaUpdate(new SysScheduleModel())
                        .eq(SysScheduleModel::getId, model.getId())
                        .eq(SysScheduleModel::getNamespace, model.getNamespace())
        ) == 1) {
            try {
                // 更新成功 提交任务
                ...
                submit(model.getId(), model.getCreatedTime(), 0,
    ...
    @Pointcut("@annotation(com.keray.common.schedule.KSchedule)")
     ...
     * @throws
     */
    public boolean nowExec(Long scheduleId) {
     ...
     * @throws
     */
    public boolean cancel(Long scheduleId) {
    ...
    private boolean saveSchedule(KSchedule kSchedule, Method method, Object[] args) {
        ...
        // 提交任务
        submit(sysScheduleModel.getId(), submitNow, 0,
     ...
     * @param args      方法参数
     */
    private void submit(Long id, LocalDateTime startTime, int retryCount,
                        String kz, String cron, String beanName, int retryMaxCount, int retryMS,
                        Method method, Object[] args) {
        log.info("提交任务:id={},startTime={},retryCount={},namespace={},kz={},cron={},beanName={},retryMaxCount={},retryMS={},method={},args={}",
                id, startTime, retryCount, namespace, kz, cron, beanName, retryMaxCount, retryMS, method, args
        );
                ...
                try {
                    ...
                    if (exec(id, beanName, method, args, model)) {
                        ...
                        if (retryMaxCount > 0) {
                            ...
                            submit(id, startTime.minus(-retryMS, ChronoUnit.MILLIS), retryMaxCount, kz, cron, beanName, retryCount + 1, retryMS, method, args);
                        } else {
                            updateScheduleStatus(id, ScheduleStatus.fail);
                        }
                    }
     ...
     * @throws
     */
    private boolean exec(long id, String beanName, Method method, Object[] args, SysScheduleModel oldModel) throws InvocationTargetException, IllegalAccessException, TimeoutException, InterruptedException {
        var lock = distributedLock.tryLock("k-schedule" + id);
        try {
            SysScheduleModel old = sysScheduleMapper.selectById(id);
            if (old.getStatus() != ScheduleStatus.waitExec) {
